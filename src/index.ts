...
// pathがflatになった
import a from 'date-fns/addDays/index'
...
try {
    const isOverlap = areIntervalsOverlapping(
        ...
        { start: end2, end: start2 }    // ここがstartの方が遅いくて不正
    )
    console.log({ isOverlap }); // 呼ばれない
} catch (err: any) {
...
try {
    const ret = roundToNearestMinutes(new Date('2024/04/02 20:30:12'), { nearestTo: -1});
    console.log(ret) // 呼ばれない
} catch (err:any) {
